...
from homeassistant_enocean.device_factories.device_factory import EnOceanDeviceFactory
from homeassistant_enocean.device_type import EnOceanDeviceType
from homeassistant_enocean.devices.f602xx_device import EnOceanF602XXDevice
...
class EnOceanF602XXDeviceFactory(EnOceanDeviceFactory):
    ...
    def create_device(self, enocean_id: EnOceanDeviceAddress, device_type: EnOceanDeviceType, send_packet: EnOceanSendRadioPacket | None = None, device_name: str | None = None, sender_id: EnOceanAddress=None) -> EnOceanF602XXDevice:
        ...
        if device_type.eep == EEP(0xF6, 0x02, 0x01) or device_type.eep == EEP(0xF6, 0x02, 0x02):
            return EnOceanF602XXDevice(enocean_id=enocean_id, device_type=device_type, send_packet=send_packet, device_name=device_name, sender_id=sender_id)
        else:
            raise ValueError(f"EEP {device_type.eep} is not supported by EnOceanF602XXDeviceFactory.")