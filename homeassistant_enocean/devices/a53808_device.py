...
class EnOceanA53808Device(EnOceanDevice):
    ...
    def light_turn_on(self, entity_uid: EnOceanEntityUID, brightness: int | None = None, color_temp_kelvin: int | None = None) -> None:
        ...
        packet = RadioPacket.create(
            ...
            sender=self.sender_id.to_bytelist(),
            COM=CMD_DIMMING, # command 2 (set dimmer)
            EDIM=100*self.convert_absolute_home_assistant_brightness_to_relative_device_brightness(brightness),
            RMP=self.__ramping_time,
            EDIMR=1,
            STR=0,
            SW=1
    ...
    def convert_relative_device_brightness_to_absolute_home_assistant_brightness(self, relative_device_brightness: float) -> float:
        ...
        ha_brightness = ((100.0*relative_device_brightness - self._min_brightness) / self._brightness_range) * 255.0
