...
class EnOceanGatewayDevice(EnOceanDevice):
    ...
    def initialize_entities(self) -> None:
        ...
        self._button_entities = [
            HomeAssistantEntityProperties(unique_id="toggle_learning", entity_category="diagnostic"),
        ]
        ...
        self.__learning_id = EnOceanAddress(self.__base_id["value"]) if self.__base_id else None
        

        self._binary_sensor_entities = [
            HomeAssistantEntityProperties(unique_id="learning", entity_category="diagnostic"),
        ]

        self._select_entities = [
            HomeAssistantEntityProperties(unique_id="sender_id", entity_category="diagnostic", options=valid_sender_ids, current_option=self.__base_id["label"] if self.__base_id else None),
        ]

        self._sensor_entities = [
            HomeAssistantEntityProperties(unique_id="learning_countdown", device_class="duration", native_unit_of_measurement="s", entity_category="diagnostic"),
        ]
    ...
    def teach(self, packet: RadioPacket, send: Callable[[RadioPacket], None]) -> EnOceanAddress | None:
        ...
        if self.__learning_mode_active and self.__learning_id:
            ...
            if isinstance(packet, UTETeachInPacket):
                ...
                device_eep = EEP(rorg=packet.rorg_of_eep, func=packet.rorg_func, type_=packet.rorg_type)
                print(f"Received UTE Teach-In packet from {device_address.to_string()} with EEP {device_eep.to_string()}.")
                response = packet.create_response_packet(self.__learning_id.to_bytelist())
                print(f"Responding with UTE Teach-In response packet to {device_address.to_string()} using sender id {self.__learning_id.to_string()}.")
                send(response)
                self.stop_learning()
            ...
            if packet.rorg == 0xF6 and packet.learn:
                print("UNSUPPORTED: Received 4BS Teach-In packet from " + EnOceanAddress(packet.sender_hex).to_string() + ".")
                self.stop_learning()
                return
    ...
    def press_button(self, entity_uid: EnOceanEntityUID) -> None:
        ...
        if entity_uid == "toggle_learning":
            ...
            if self.__learning_mode_active:
                ...
                if learning_countdown_callback := self._sensor_callbacks.get("learning_countdown"):
            ...
            if self._valid_sender_ids:
                for option_dict in self._valid_sender_ids:
                    if option_dict["label"] == option:
                        ...
                        print(f"Gateway learning ID set to {self.__learning_id.to_string()}.")
                        break
