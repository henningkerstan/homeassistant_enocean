...
async def main_loop():
    ...
    for device in devices:
        ...
        gateway.add_device(
            enocean_id=EnOceanAddress(device["enocean_id"]),
    ...
    for binary_sensor in gateway.binary_sensor_entities:
        print(
            f"Registered binary sensor entity: {binary_sensor.to_string()} with properties: {gateway.binary_sensor_entities[binary_sensor]}"
        )
        gateway.register_binary_sensor_callback(
            binary_sensor,
            lambda is_on, entity_id=binary_sensor: binary_sensor_callback(
                entity_id, is_on
            ),
        )
    ...
    for cover in gateway.cover_entities:
        print(
            f"Registered cover entity: {cover.to_string()} with properties: {gateway.cover_entities[cover]}"
        )
        gateway.register_cover_callback(
            cover, lambda position, entity_id=cover: cover_callback(entity_id, position)
        )

    for sensor in gateway.sensor_entities:
        print(
            f"Registered sensor entity: {sensor.to_string()} with properties: {gateway.sensor_entities[sensor]}"
        )
        gateway.register_sensor_callback(
            sensor, lambda value, entity_id=sensor: sensor_callback(entity_id, value)
        )

    for switch in gateway.switch_entities:
        print(
            f"Registered switch entity: {switch.to_string()} with properties: {gateway.switch_entities[switch]}"
        )
        gateway.register_switch_callback(
            switch, lambda is_on, entity_id=switch: switch_callback(entity_id, is_on)
        )
